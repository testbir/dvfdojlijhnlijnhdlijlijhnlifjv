...
async def create_module(course_id: int, data: ModuleCreate, current_admin: AdminUser = Depends(get_current_admin_user)):
...
async def retrieve_module(module_id: int, current_admin: AdminUser = Depends(get_current_admin_user)):
    return await get_module(module_id)

@router.put("/admin/modules/{module_id}", summary="Обновить модуль по ID")
async def edit_module(module_id: int, data: ModuleCreate, current_admin: AdminUser = Depends(get_current_admin_user)):
    return await update_module(module_id, data)

@router.delete("/admin/modules/{module_id}", summary="Удалить модуль по ID")
async def remove_module(module_id: int, current_admin: AdminUser = Depends(get_current_admin_user)):
    return await delete_module(module_id)


@router.get("/admin/courses/{course_id}/modules/", summary="Получить список модулей для курса")
async def list_modules_for_course(course_id: int, current_admin: AdminUser = Depends(get_current_admin_user)):
